import { useState, useEffect } from 'react';
import api from '../../services/api';

export default function AchievementsAdmin() {
  const [achievements, setAchievements] = useState([]);
  const [formData, setFormData] = useState({ id: null, judul: '', deskripsi: '', tahun: '', gambar: null });
  const [isEditing, setIsEditing] = useState(false);

  const fetchData = async () => {
    try {
      const res = await api.get('/admin/achievements');
      setAchievements(res.data);
    } catch (err) { console.error(err); }
  };

  useEffect(() => { fetchData(); }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();
    const data = new FormData();
    data.append('judul', formData.judul);
    if (formData.deskripsi) data.append('deskripsi', formData.deskripsi);
    if (formData.tahun) data.append('tahun', formData.tahun);
    if (formData.gambar) data.append('gambar', formData.gambar);

    try {
      if (isEditing) {
        await api.put(`/admin/achievements/${formData.id}`, data);
      } else {
        await api.post('/admin/achievements', data);
      }
      setFormData({ id: null, judul: '', deskripsi: '', tahun: '', gambar: null });
      setIsEditing(false);
      fetchData();
    } catch (err) { alert('Gagal menyimpan prestasi'); }
  };

  const handleEdit = (a) => {
    setFormData({ id: a.id, judul: a.judul, deskripsi: a.deskripsi || '', tahun: a.tahun || '', gambar: null });
    setIsEditing(true);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Yakin hapus?')) {
      await api.delete(`/admin/achievements/${id}`);
      fetchData();
    }
  };

  return (
    <div>
      <h1 className="text-3xl font-bold mb-6">Kelola Prestasi</h1>
      <div className="bg-white p-6 rounded shadow-md mb-8">
        <h2 className="text-xl font-semibold mb-4">{isEditing ? 'Edit' : 'Tambah'} Prestasi</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-sm">Judul</label><input type="text" className="w-full border p-2" value={formData.judul} onChange={e => setFormData({...formData, judul: e.target.value})} required /></div>
            <div><label className="block text-sm">Tahun</label><input type="number" className="w-full border p-2" value={formData.tahun} onChange={e => setFormData({...formData, tahun: e.target.value})} /></div>
          </div>
          <div><label className="block text-sm">Deskripsi</label><textarea className="w-full border p-2" value={formData.deskripsi} onChange={e => setFormData({...formData, deskripsi: e.target.value})} /></div>
          <div><label className="block text-sm">Gambar</label><input type="file" accept="image/*" onChange={e => setFormData({...formData, gambar: e.target.files[0]})} /></div>
          <div className="flex gap-2">
            <button className="bg-blue-600 text-white px-4 py-2">Simpan</button>
            {isEditing && <button type="button" onClick={() => { setIsEditing(false); setFormData({ id: null, judul: '', deskripsi: '', tahun: '', gambar: null }); }} className="bg-gray-400 text-white px-4 py-2">Batal</button>}
          </div>
        </form>
      </div>
      <table className="min-w-full bg-white shadow-md rounded">
        <thead className="bg-gray-50"><tr><th className="px-6 py-3 text-left">Judul</th><th className="px-6 py-3 text-left">Tahun</th><th className="px-6 py-3 text-left">Aksi</th></tr></thead>
        <tbody>
          {achievements.map(a => (
            <tr key={a.id} className="border-t">
              <td className="px-6 py-4">{a.judul}</td>
              <td className="px-6 py-4">{a.tahun}</td>
              <td className="px-6 py-4">
                <button onClick={() => handleEdit(a)} className="text-indigo-600 mr-4">Edit</button>
                <button onClick={() => handleDelete(a.id)} className="text-red-600">Hapus</button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
}
